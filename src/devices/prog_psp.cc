...
unsigned short icsp_write(_pic *pic,int type,unsigned short value)
{
  ...
  {
    case TPROG:
      if(prog_addr < pic->ROMSIZE)pic->prog[prog_addr]=value;
      prog_addr+=1;
      break;
    case TEEPROM:
      if(eeprom_addr < pic->EEPROMSIZE)pic->eeprom[eeprom_addr]=value;
      eeprom_addr+=1;
      break;
    case TUID:
      if(uid_addr < pic->IDSIZE)pic->id[uid_addr]=value;
      uid_addr+=1;
      break;
    case TCONFIG:
      if(config_addr < pic->CONFIGSIZE)pic->config[config_addr]=value;
      config_addr+=1;
      break;
    default: 
      break;
...
unsigned short ret=0;
  ...
  {
    case TPROG: 
      if(prog_addr < pic->ROMSIZE)ret=pic->prog[prog_addr];
      prog_addr+=1;
      break;
    case TEEPROM:
      if(eeprom_addr < pic->EEPROMSIZE)ret=pic->eeprom[eeprom_addr];
      eeprom_addr+=1;
      break;
    case TUID:
      if(uid_addr < pic->IDSIZE)ret=pic->id[uid_addr];
      uid_addr+=1;
      break;
    case TCONFIG:
      if(config_addr < pic->CONFIGSIZE)ret=pic->config[config_addr];
      config_addr+=1;
      break;
    default: 
      break;
...
int prog_loop(_pic * pic)
{
  ...
  unsigned char aux;
		...
		for(i=0;i< info.mclen;i++)
                  if(icsp_read(pic,TPROG)!=info.dww)aux=1;
		...
		for(i=0;i< info.el;i++)
                  if(icsp_read(pic,TEEPROM)!=info.mem)aux=1;
		...
		for(i=0;i< info.uil;i++)
                  if(icsp_read(pic,TUID)!=info.uim)aux=1;
		...
		for(i=0;i<  info.cl ;i++)
                  if(icsp_read(pic,TCONFIG)!= ecm[i])aux=1;
